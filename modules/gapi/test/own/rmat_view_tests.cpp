...
struct RMatViewTest : public TestWithParam<int>{};
...
static Mat wrapViewByMat(View& view) {
    return Mat(view.size(), view.type(), view.ptr(), view.step());
}

TEST_P(RMatViewTest, StepIn) {
    auto type = GetParam();
    ...
    const auto viewMat = wrapViewByMat(view);
    Mat ref, out;
    cv::Size ksize{1,1};
...
TEST_P(RMatViewTest, StepOut) {
    ...
    auto viewMat = wrapViewByMat(view);
    Mat ref;
    cv::Size ksize{1,1};
